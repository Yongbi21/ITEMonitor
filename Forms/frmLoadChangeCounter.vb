Public Class frmLoadChangeCounter
...
#Region "Calling Function"
        ...
        Private Sub loadMain(Optional searchTerm As String = "")
            ...
            Else
                ...
                query = $"EXEC GetChangeCounterHistory @SearchTerm = '{searchTerm.Replace("'", "''")}'"
            End If

                tbl = da.GetLoadData(dbName, query)

                If tbl.Rows.Count > 0 Then
                bsChangeCounterLoad.DataSource = tbl
            Else
                bsChangeCounterLoad.DataSource = Nothing
            End If
...
#End Region
        ...
        Private Sub DataGridViewX1_KeyDown(sender As Object, e As KeyEventArgs) Handles DataGridViewX1.KeyDown
            ...
            If bsChangeCounterLoad.Current IsNot Nothing Then
                selectedArea = CType(bsChangeCounterLoad.Current, DataRowView)("Change_Counter_Id").ToString()
                Me.Close()
            End If
        End If
        End Sub
