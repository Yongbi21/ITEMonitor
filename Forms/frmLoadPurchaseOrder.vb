Public Class frmLoadPurchaseOrder

    Private da As New clDataAceesV2("DEVJB\SQLEXPRESS", "SGCuser", "Syst3ms")
    Private Const dbName As String = "ITEMonitordb"
    Public selectedPurchaseOrderId As String
    Public selectedPurchaseOrderNumber As String
    Public selectedPurchaseOrderDate As String
    Public selectedInvoiceNumber As String
    Public selectedInvoiceDate As String
...
#Region "Calling Function"
    Private Sub loadMain()
        ...
        tbl = da.GetLoadData(dbName, "SELECT * FROM [PurchaseOrder]")
        If tbl.Rows.Count > 0 Then
            bsPurchaseOrderLoad.DataSource = tbl
        Else
            bsPurchaseOrderLoad.DataSource = Nothing
        End If

    End Sub
#End Region

    Private Sub frmLoadPurchaseOrder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub SearchBox_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        ...
        If Not String.IsNullOrWhiteSpace(txtSearch.Text) Then
            filter = String.Format("Purchase_Order_Number LIKE '%{0}%'", txtSearch.Text.Replace("'", "''"))
        End If
        bsPurchaseOrderLoad.Filter = filter
    End Sub
    ...
    Private Sub SelectAndClose()
        If bsPurchaseOrderLoad.Current IsNot Nothing Then
            Dim currentRow As DataRowView = CType(bsPurchaseOrderLoad.Current, DataRowView)
            selectedPurchaseOrderId = currentRow("Purchase_Order_Id").ToString()
            selectedPurchaseOrderNumber = currentRow("Purchase_Order_Number").ToString()
            selectedPurchaseOrderDate = currentRow("Purchase_Order_Date").ToString()
    ...
    Private Sub enterBtnCounter_Click(sender As Object, e As EventArgs) Handles enterBtnPurchaseOrder.Click
        SelectAndClose()
    End Sub
